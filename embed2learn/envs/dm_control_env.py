...
import numpy as np
from embed2learn.envs.dm_control_viewer import DmControlViewer
...
class DmControlEnv(Env):
	...
	def action_space(self):
		...
		return Box(low=np.min(action_spec.minimum),
	...
	def step(self, action):
		...
		return Step(observation=self.__observe(), reward=reward, done=step.last())

	def render(self):
		# TODO(rjulian): real MjViewer support
		if not self.viewer:
			self.viewer = DmControlViewer()
		image = self.dm_env.physics.render(RENDER_HEIGHT, RENDER_WIDTH, RENDER_CAMERA_ID)
