...
class VectorizedSampler(BatchSampler):
    def __init__(self, algo, n_envs=None, es=None):
    ...
    def start_worker(self):
        ...
        if getattr(self.algo.env, 'vectorized', False):
            ...
            print(self.algo.env)
            if isinstance(self.algo.env.env, EmbeddedPolicyEnv):
                envs = [
                    EmbeddedPolicyEnv(pickle.loads(pickle.dumps(self.algo.env.env._wrapped_env)), self.algo.env.env._wrapped_policy)
    ...
    def obtain_samples(self, itr):
        ...
        while n_samples < self.algo.batch_size:
            ...
            if self.es is None:
                ...
                actions = self.es.get_actions(obses)
